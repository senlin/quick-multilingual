jQuery(document).ready(function($) {
        ...
        ------------------------------------------------------- */
        var $langCountSelect = $('#so_qmp_number_of_languages:not([disabled])');
        if ($langCountSelect.length) {
                // On page load, ensure disabled state of inputs in hidden rows matches their visibility
                $('.so_qmp-extra-lang-row').each(function() {
                        if ($(this).attr('hidden') !== undefined) {
                                $(this).find('input').prop('disabled', true);
                        }
                });

                $langCountSelect.on('change', function() {
                        var count = parseInt($(this).val(), 10);
                        $('.so_qmp-extra-lang-row').each(function() {
                                var langIndex = parseInt($(this).data('lang-index'), 10);
                                if (langIndex <= count) {
                                        $(this).removeAttr('hidden').find('input').prop('disabled', false);
                                } else {
